...
# Periksa apakah dataset memiliki kolom yang benar
...
# Gunakan MinMaxScaler untuk menjaga proporsi warna
scaler = MinMaxScaler()
...
while True:
    ...
    # Konversi BGR ke RGB agar sesuai dengan dataset
    ...
    roi1 = frame_rgb[y1:y1+box_size, x1:x1+box_size]
    roi2 = frame_rgb[y2:y2+box_size, x2:x2+box_size]
    
    # Ambil rata-rata warna dalam bounding box
    pixel1 = np.mean(roi1, axis=(0, 1))
    pixel2 = np.mean(roi2, axis=(0, 1))
    
    # Normalisasi nilai pixel agar sesuai dengan model
    ...
    # Temukan warna asli terdekat (setelah normalisasi)
    distances1 = np.linalg.norm(X_scaled - pixel1_scaled, axis=1)
    distances2 = np.linalg.norm(X_scaled - pixel2_scaled, axis=1)
    ...
    cv2.putText(frame, f'Color1: {color_pred_1} | Accuracy1: {realtime_accuracy_1:.2f}%', (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(frame, f'Color2: {color_pred_2} | Accuracy2: {realtime_accuracy_2:.2f}%', (10, 60), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    
    # Gambar bounding box pada frame dengan keterangan
    cv2.rectangle(frame, (x1, y1), (x1+box_size, y1+box_size), (255, 255, 255), 2)
    cv2.rectangle(frame, (x2, y2), (x2+box_size, y2+box_size), (255, 255, 255), 2)
    cv2.putText(frame, "Color 1", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    cv2.putText(frame, "Color 2", (x2, y2 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 25), 2)
    
    print(f'Color1: {color_pred_1} | Accuracy1: {realtime_accuracy_1:.2f}%')
    print(f'Color2: {color_pred_2} | Accuracy2: {realtime_accuracy_2:.2f}%')
