...
def find_nearest_color(avg_color_rgb): #avg_color_rgb sudah dalam format RGB
    distances = np.linalg.norm(X - avg_color_rgb, axis=1)
...
while True:
    ...
    avg_color_bgr = np.mean(roi, axis=(0, 1)).astype(int)  # Ambil warna rata-rata dalam BGR
    
    # Konversi BGR ke RGB
    avg_color_rgb = avg_color_bgr[::-1] #membalik array agar menjadi RGB
    ...
    true_color = find_nearest_color(avg_color_rgb) #gunakan RGB
    ...
    print(f"Deteksi Warna: {color_pred} | Warna Asli: {true_color} | Akurasi: {color_accuracy:.2f}%")
    
    # Gambar bounding box
    cv2.rectangle(frame, (x_start, y_start), (x_end, y_end), (0, 0, 0), 2)
    
    # Tambahkan label warna pada bounding box
    cv2.putText(frame, f'{color_pred}', (x_start, y_start - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
    
    # Tampilkan informasi warna dan akurasi
    cv2.putText(frame, f'Color: {color_pred}', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'True Color: {true_color}', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'Accuracy: {color_accuracy:.2f}%', (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
